...
- (id)initWithPath:(NSString *)path
{
	...
	if ((self = [super init]))
	{
		...
		// If file is not present query web service.
		//if(objData == nil){
		//	NSString * urlFile = [NSString stringWithFormat:@"http://subjectreality.appspot.com/getfile?id=%@", path];
		//	NSData * fileData = [[NSData alloc] initWithContentsOfURL: [NSURL URLWithString:urlFile]];
		//	objData = [[NSString alloc] initWithData:fileData encoding:NSASCIIStringEncoding]; 
		//}
		...
		for (NSString * line in lines)
		{
			...
			else if ([line hasPrefix:@"m"])
			{
				...
				if(mtlPath == nil){
					mtlPath = [NSString stringWithFormat:@"%@", [truncLine lastPathComponent]];
				}
