...
static inline Color3D Color3DMake_(CGFloat inRed, CGFloat inGreen, CGFloat inBlue, CGFloat inAlpha)
{
    Color3D ret;
	ret.red = inRed;
	ret.green = inGreen;
...
static inline Vertex3D Vertex3DMake_(CGFloat inX, CGFloat inY, CGFloat inZ)
{
	Vertex3D ret;
	ret.x = inX;
	ret.y = inY;
...
#pragma mark -
typedef Vertex3D Vector3D_;
#define Vector3DMake(x,y,z) (Vector3D)Vertex3DMake(x, y, z)
static inline GLfloat Vector3DMagnitude_(Vector3D vector)
{
	return sqrt((vector.x * vector.x) + (vector.y * vector.y) + (vector.z * vector.z)); 
}
static inline void Vector3DNormalize_(Vector3D *vector)
{
...
static inline GLfloat Vector3DDotProduct_(Vector3D vector1, Vector3D vector2)
{		
	return vector1.x*vector2.x + vector1.y*vector2.y + vector1.z*vector2.z;
	
}
static inline Vector3D Vector3DCrossProduct_(Vector3D vector1, Vector3D vector2)
{
	Vector3D ret;
...
static inline Vector3D Vector3DMakeWithStartAndEndPoints_(Vertex3D start, Vertex3D end)
{
	Vector3D ret;
	ret.x = end.x - start.x;
	ret.y = end.y - start.y;
...
static inline Vector3D Vector3DAdd_(Vector3D vector1, Vector3D vector2)
{
	Vector3D ret;
...
static inline void Vector3DFlip_ (Vector3D *vector)
{
	vector->x = -vector->x;
	vector->y = -vector->y;
	vector->z = -vector->z;
...
static inline Face3D Face3DMake_(int v1, int v2, int v3)
{
	Face3D ret;
	ret.v1 = v1;
	ret.v2 = v2;
...
static inline Triangle3D Triangle3DMake_(Vertex3D inV1, Vertex3D inV2, Vertex3D inV3)
{
	Triangle3D ret;
	ret.v1 = inV1;
	ret.v2 = inV2;
...
static inline Vector3D Triangle3DCalculateSurfaceNormal_(Triangle3D triangle)
{
...
static inline VertexTextureIndex * VertexTextureIndexMake_ (GLuint inVertex, GLuint inTextureCoords, GLuint inActualVertex)
{
...
static inline GLuint VertexTextureIndexMatch_(VertexTextureIndex *node, GLuint matchVertex, GLuint matchTextureCoords)
{
...
static inline VertexTextureIndex * VertexTextureIndexAddNode_(VertexTextureIndex *node, GLuint newVertex, GLuint newTextureCoords)
{
...
static inline BOOL VertexTextureIndexContainsVertexIndex_(VertexTextureIndex *node, GLuint matchVertex)
{
...
static inline void VertexTextureIndexFree_(VertexTextureIndex *node)
{
	if (node != NULL)
	{
		if (node->greater != NULL)
...
static inline GLuint VertexTextureIndexCountNodes_(VertexTextureIndex *node)
{
	GLuint ret = 0;  
